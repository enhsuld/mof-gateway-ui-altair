	   		   				...
	   		   				if(data.rread==1){
		   		   		       ...
		   		   		       $scope.puserGrid = {
		   			                ...
		   			                columns: [
		   			                          ...
		   			                          { field:"regnum", title: "Байгууллагын регистр",width:"200px",locked: true,},	
	   		   					...
	   		   					if(data.rupdate==1){	   		   						
	   		   						$scope.puserGrid.columns.push({template: kendo.template($('#update').html()),  width: 135})
	   		   					}
	   		   					if(data.rdelete==1){
	   		   						$scope.puserGrid.columns.push({command:["destroy"],  width: 140})
	   		   					}
	   		   				}
	   		   				else{
	   		   					$state.go('error.404');
