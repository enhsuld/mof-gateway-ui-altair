angular
    .module('altairApp')
    	.controller("mgtSettingsCtrl",['$rootScope','$scope','orgs','mainService','$state','sweet','$cookies',
	        function ($rootScope,$scope,orgs,mainService,$state,sweet,$cookies) {
                ...
                $scope.domain="com.macro.dev.models.SettingsMgtCompany.";
				...
				$scope.pmenuGrid = {
					dataSource: {
						...
						transport: {
							read:  {
								url: "/api/core/list/SettingsMgtCompany?access_token="+$cookies.get('access_token'),
							...
							total:"total",
							 model: {
								 ...
								 fields: {
                                     ...
                                     egno: { type: "number", validation: { required: true } },
									 orgid: { type: "number", validation: { required: true } },
									 autoinc: { type: "boolean",validation: { required: true } },
								 }
							 }
						 },
						pageSize: 8,
			...
			if($rootScope.ruptype==1){
                ...
                if($rootScope.rexport==1){
                    ...
                    $scope.pmenuGrid.toolbar.push({template: kendo.template($("#template").html()), width: "200px"});
                }
